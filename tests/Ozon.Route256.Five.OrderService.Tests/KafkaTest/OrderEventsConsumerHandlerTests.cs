...
public class OrderEventsConsumerHandlerTests
{
    ...
    public async Task HandleAsync_OrderNotFound_ThrowsException()
    {
        ...
        _orderRepositoryMock.Setup(x => x.FindAsync(It.IsAny<long>(), It.IsAny<CancellationToken>())).ReturnsAsync((Order)null);
    ...
    public async Task HandleAsync_OrderFound_UpdatesOrder()
    {
        ...
        var order = new Order { Id = 1, State = OrderState.Created, DateUpdate = DateTime.UtcNow };
